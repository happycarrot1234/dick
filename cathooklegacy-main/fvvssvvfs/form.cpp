...
void Form::draw( ) {
	...
	if (GetAsyncKeyState(VK_LBUTTON)) {
		...
		if (cursorpos.x > m_x + m_width - 15 && cursorpos.x < m_x + m_width + 15)
			x_right = true;

		if (cursorpos.y > m_y + m_height - 15 && cursorpos.y < m_y + m_height + 15)
			y_right = true;

		int x = cursorpos.x - m_x;
		int y = cursorpos.y - m_y;
		...
		if ((x_right && y_right) || mousePressed) {
			mousePressed = true;
			SetSize(x, y);
		}
	}

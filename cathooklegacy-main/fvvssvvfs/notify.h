...
public:
	...
	void think(Color nigga) {
		...
		for (size_t i{}; i < m_notify_text.size(); ++i) {
			...
			std::string prefix = XOR("[ CH ]");
			render::FontSize_t fontsize = render::logevent.size(prefix);
			render::logevent.string(x, y, nigga, prefix);
			render::logevent.string(x + fontsize.m_width + 5, y, color, notify->m_text);
			y += size;
		}
	}
};

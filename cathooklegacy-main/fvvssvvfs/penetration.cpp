...
void UTIL_TraceLineIgnoreTwoEntities(const vec3_t& vecAbsStart, const vec3_t& vecAbsEnd, uint32_t mask,
	Entity* ignore, Entity* ignore2, int collisionGroup, CGameTrace* ptr)
{
	static CTraceFilterSkipTwoEntities_game filter{};
	filter.SetPassEntity(ignore);
	filter.SetPassEntity2(ignore2);
	g_csgo.m_engine_trace->TraceRay(Ray(vecAbsStart, vecAbsEnd), mask, (ITraceFilter*)&filter, ptr);
}

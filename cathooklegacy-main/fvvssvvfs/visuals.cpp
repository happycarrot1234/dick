...
void Visuals::DrawPlayer(Player* player) {
	...
	if (health_esp) {
		...
		if (player_hp[player->index()] > hp)
			player_hp[player->index()] -= 100.f * g_csgo.m_globals->m_frametime;
		else
		...
		// render actual bar.
		if (dormant)
			render::rect(box.x - 3, y + h - fill - 1, 2, fill + 2, { 110, 130, 110 , alpha });
		else
			render::rect(box.x - 3, y + h - fill - 1, 2, fill + 2, { r, g, 0, alpha });
